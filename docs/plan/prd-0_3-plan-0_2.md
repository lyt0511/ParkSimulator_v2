...
### 切片1
在【`IDLE/READY`】下，当【用户进入练习页并选择场景】发生时，系统通过【`play-page -> scene-loader -> scene-renderer`】在【主画布】呈现【黑色道路、白色标线、停车位划线、绿化带、静态参照车的基础场景图像】。  
...
### 切片2
在【`READY/RUNNING`】下，当【用户键盘/按钮输入控制】发生时，系统通过【`input-dispatcher -> input-guard -> session-controller -> scene-renderer`】在【主画布 + HUD】呈现【自车以“车头+车体+车轮”形态运动与转向，且控制实时可见】。  
